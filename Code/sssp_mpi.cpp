...
int main(int argc, char** argv) {
    ...
    vector<int> dist(max_node + 1, INF), parent(max_node + 1, -1);

    // Only rank 0 computes full SSSP
    if (rank == 0) {
        auto start = high_resolution_clock::now();
        recompute_sssp(adj, dist, parent, source);
        auto end = high_resolution_clock::now();
        cout << "Initial SSSP completed in "
             << duration_cast<milliseconds>(end - start).count() << " ms\n";
    }

    // Broadcast dist and parent to all
    broadcast_vector(dist, 0);
    broadcast_vector(parent, 0);
